...
{
    {
        ...
        for(size_t i=0;i<sz/2;++i)
        {
            ...
            EXPECT_EQ(v.at(i), (int)i+1);

        }
        EXPECT_FALSE(v.empty());
        EXPECT_EQ(v.size(), sz/2);
        EXPECT_EQ(std::accumulate(v.begin(), v.end(), 0U), (sz/2 * (sz/2+1)) /2 );

        auto pos = v.insert(v.end(), {6,7,8,9,10});
        EXPECT_EQ(*pos, 6);
        EXPECT_EQ(v.size(), sz);
        EXPECT_EQ(std::accumulate(v.begin(), v.end(), 0U), (sz * (sz+1)) /2 );
        ...
        v.assign(6U, 11); // assign(size_type n, const T& u)
        EXPECT_EQ(v.size(), 6U);
        EXPECT_EQ(std::accumulate(v.begin(), v.end(), 0), 66);
    
        v.assign({1,9}); // assign(std::initializer_list<T> il) => assign(InputIterator first, InputIterator last)
        EXPECT_EQ(v.size(), 2U);
