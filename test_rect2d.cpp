...
TEST(Shape2d, Rect32_Basic)
{
    ...
    constexpr Rect32 r0;
    constexpr Rect32 r1(1,2,3,4); // Rectangle(T x, T y, T w, T h)
    Rect32 r2(Rect32(10,20,30,80));
    constexpr Rect32 r22(Pos2i(10,20), Pos2i(39, 99)); // Rectangle(const Point<T>& lt, const Point<T>& rb)
    constexpr Rect32 r23(Pos2i(10,20), 30, 80); // Rectangle(const Point<T>& pos, const T& w, const T& h)
    constexpr Rect32 max_r(Rect32(int32_min, int32_min, int32_max, int32_max));

    EXPECT_EQ(r0.left(), 0);
    EXPECT_EQ(r0.top(), 0);
    EXPECT_EQ(r0.width(), 0);
    EXPECT_EQ(r0.height(), 0);

    EXPECT_EQ(r1.left(), 1);
    EXPECT_EQ(r1.top(), 2);
    EXPECT_EQ(r1.right(), 3);
    EXPECT_EQ(r1.bottom(), 5);
    EXPECT_EQ(r1.width(), 3);
