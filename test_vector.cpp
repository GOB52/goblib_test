...
TEST(Vector2, Basic)
{
    ...
    GOBLIB_CONSTEXPR_GCC auto isvalid = v1.valid();
    constexpr auto lensq = v1.lengthSq();
    GOBLIB_CONSTEXPR_GCC auto angle = v1.angle();
...
TEST(Vector2, infinity)
{
    ...
    EXPECT_TRUE(vec[1] == vec[1]);
    EXPECT_TRUE(vec[2] == vec[2]);
    EXPECT_TRUE(vec[3] == vec[3]);
