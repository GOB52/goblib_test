...
class A
{
  public:
    constexpr A() : _va(0){}
    int32_t value_a() const { return _va; }
  private:
    std::int32_t _va;
};
...
class B : public A
{
  public:
    constexpr B() : A(),_vb(1){}
    int32_t value_b() const { return _vb; }
  private:
    std::int32_t _vb;
};
...
class C : public B
{
  public:
    constexpr C() : B(), _vc(2){}
    int32_t value_c() const { return _vc; }
  private:
    std::int32_t _vc;
};
...
TEST(Utility, size)
{
    ...
    EXPECT_EQ(1U, goblib::size(vector_c0));
    vector_c0.push_back(C());
    EXPECT_EQ(2U, goblib::size(vector_c0));
    vector_c0.pop_back();
    EXPECT_EQ(1U, goblib::size(vector_c0));
    vector_c0.pop_back();
    ...
    EXPECT_EQ(2U, goblib::size(map_0));
...
namespace
{
template<size_t N> void test_int_by_size()
{
    using myint_t = goblib::int_by_size_t<N>;
    EXPECT_EQ(sizeof(myint_t), (N ? (goblib::math::roundUpPow2(N) + 7) / 8U : 1)) << "N = " << N;

    using umyint_t = goblib::uint_by_size_t<N>;
    EXPECT_EQ(sizeof(umyint_t), (N ? (goblib::math::roundUpPow2(N) + 7) / 8U : 1)) << "N = " << N;
}

template<typename T> void test_next_int()
{
    using next_int_t = goblib::int_by_size_t<sizeof(T) * CHAR_BIT + 1>;

    EXPECT_GT(sizeof(next_int_t), sizeof(T)) << TypeName<T>();
    EXPECT_TRUE(std::is_signed<next_int_t>::value);
    ...
    EXPECT_GT(sizeof(next_uint_t), sizeof(UT)) << TypeName<UT>();
    EXPECT_TRUE(std::is_unsigned<next_uint_t>::value);
    EXPECT_EQ(std::is_signed<next_uint_t>::value, std::is_signed<UT>::value) << TypeName<UT>();
}
//
}

TEST(Utility, int_by_size)
{
    test_int_by_size<0>();
    test_int_by_size<1>();
    test_int_by_size<7>();
