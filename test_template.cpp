...
struct HasLess  { bool operator< (const HasLess&)  { return true; } }; // class has <
struct HasEqual { bool operator==(const HasEqual&) { return true; } }; // class has ==

struct HasLessAndEqual{ int val() const { return v; } private: int v; }; // support ==,< by global function.
bool operator==(const HasLessAndEqual&, const HasLessAndEqual&) { return true; }
bool operator< (const HasLessAndEqual& a, const HasLessAndEqual& b) { return a.val() <  b.val(); }

struct Base {};
struct Delivered : public Base {};
int foo(const char*, int, float = 1.0f, void*  = nullptr) { return 42; }
void bar(...) {}
const int& foobar(Base*) { static int val; return ++val; }

struct functor
{
    int operator()(int a, int b) const { return a + b; }
    const char* operator()(const char*, const char*) const { return nullptr; }
};

template<typename T> auto temp_func(const T& v, const T&, const T&)
...
// GCC builtin std::sin can be constexpr.
namespace
{
